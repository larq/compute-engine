...
template <int LhsCols, int RhsCols>
inline void MakeBinaryKernelParams32BP(
    ...
    BinaryKernelParams32BP<LhsCols, RhsCols>* params) {
  ...
  // TODO: this should be sizeof(float)?
...
template <int LhsCols, int RhsCols>
struct BinaryKernelParams64BP {
  const std::uint64_t* lhs_base_ptr;
  const std::uint64_t* rhs_base_ptr;
  float* dst_base_ptr;
  ...
  std::uint32_t clamp_max;
  std::uint8_t flags;
  const std::uint64_t zero_data[LhsCols] = {0};
  std::uint64_t dst_tmp_buf[LhsCols * RhsCols];
};

template <int LhsCols, int RhsCols>
inline void MakeBinaryKernelParams64BP(
    const PackedMatrix<std::uint64_t>& lhs,
    const PackedMatrix<std::uint64_t>& rhs,
    const BasicSpec<std::int32_t, float>& spec, int start_row, int start_col,
    int end_row, int end_col, Matrix<float>* dst,
    BinaryKernelParams64BP<LhsCols, RhsCols>* params) {
  const int depth = lhs.layout.rows;
  RUY_DCHECK_EQ(start_row % LhsCols, 0);
  RUY_DCHECK_EQ(start_col % RhsCols, 0);
  RUY_DCHECK_EQ(end_row % LhsCols, 0);
  ...
  params->last_row = end_row - LhsCols;
  params->last_col = end_col - RhsCols;
  params->lhs_stride = sizeof(std::uint64_t) * lhs.layout.stride;
  params->rhs_stride = sizeof(std::uint64_t) * rhs.layout.stride;
