//
// This file is a copy of `tensorflow/tensorflow/lite/profiling/profile_summarizer.cc`.
// But with our custom benchmark output added to it.
// Changes compared to the original file:
// - We added something to the second half of
//      ProfileSummarizer::ProcessProfiles
//
...
void ProfileSummarizer::ProcessProfiles(
  ...
  //
  // Larq Compute Engine Profiling
  //
  events.clear();
  std::copy_if(profile_stats.begin(), profile_stats.end(),
               std::back_inserter(events), [](const ProfileEvent* e) {
                 return (e->event_type == (ProfileEvent::EventType)2) &&
                        e->end_timestamp_us >= e->begin_timestamp_us;
               });
  // Sort with begin_time.
  std::sort(events.begin(), events.end(),
  ...
  base_start_us = events[0]->begin_timestamp_us;
  for (auto event : events) {
    int64_t start_us = event->begin_timestamp_us - base_start_us;
    int64_t node_exec_time =
        event->end_timestamp_us - event->begin_timestamp_us;
    stats_calculator_->AddNodeStats(event->tag, "LQCE", node_num, start_us,
