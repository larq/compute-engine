...
TfLiteStatus ImagePreprocessingStage::Init() {
  ...
  if (output_type_ == kTfLiteUInt8) {
    ...
    // per-channel statics, but this code doesn't currently support that, and
    // empirically doing this gives 'good enough' results.
    //
    // TODO: support per-channel normalisation.
    input_mean_value_ = 0.449 * 255.0;
    scale_ = 1.0 / (0.226 * 255.0);
  } else {
