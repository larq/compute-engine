...
//      bitflip               128 |   512 |   64K |
//                           -----|-------|-------|
//  64bit y                    53 |   209 | 28900 |
//  64bit y v2                 57 |   225 | 30400 |
//  64bit y v3                 54 |   214 | 30600 |
//  64bit n                    53 |   210 | 28375 |
//  64bit n pldl1strm before   51 |   210 | 28381 |
//  64bit n pldl1strm at start 54 |   216 | 28850 |
//  64bit n pldl1keep at start 55 |   215 | 28830 |
//  64bit n pldl1strm at mid   53 |   210 | 28470 |
//  64bit n pldl1keep at mid   53 |   210 | 28380 |
// 128bit y                    54 |   213 | 28342 |
// 128bit n         `          53 |   210 | 28344 |
// 128bit n pldl1strm before   54 |   210 | 28354 |
// 128bit n pldl1keep v1       53 |   210 | 28194 |
// 128bit n pldl1strm v1       53 |   210 | 28200 |
// 128bit n pldl1strm 8times   55 |   215 | 32092 |
// 128bit n pldl1strm 4times   54 |   210 | 35930 |
//  dabnn y                    54 |   212 | 37600 |
//
// The +- indicates if it did a bitflip (not) instruction to map +1 to 1 and -1
// to 0.
//
...
void packbits_aarch64_64(const float* input, size_t num_blocks, void* output) {
  asm volatile(
      ...
      "sri    v0.4s, v2.4s, #2    \n"
      "sri    v1.4s, v3.4s, #2    \n"
      // v0,v1 contain 4 signbits in each part
      "ld1    {v2.4s, v3.4s, v4.4s, v5.4s}, [%0], #64    \n"
      "sri    v2.4s, v4.4s, #1    \n"
      "sri    v3.4s, v5.4s, #1    \n"
      "ld1    {v4.4s, v5.4s, v6.4s, v7.4s}, [%0], #64    \n"
...
//
// All functions below are for benchmarking only
//

// First we try some variations of `packbits_aarch64_64`
// In all these variations, there are:
// - 4 load instructions of 64 bytes each
// - 18 compute instructions (sri, mov, not)
// - 1 store instruction of 8 bytes
//
// We vary the order of the compute and load instructions:
//              load compute load compute load compute load compute
// Original:               2            4            2           10
// v2:                     0            4            0           14
// v3:                     3            4            3            8
//
// Timings on 64K floats:
// Original: 28900 ns    (medium balanced)
// v2:       30400 ns    (unbalanced)
// v3:       30600 ns    (very balanced)
//
// Even though v3 is more balanced than the original, it is still slower.
//

void packbits_aarch64_64_v2(const float* input, size_t num_blocks,
                            void* output) {
  asm volatile(
      //"prfm   pldl1strm, [%0]     \n"
      "0: \n"
...
void packbits_aarch64_64_v3(const float* input, size_t num_blocks,
                            void* output) {
  asm volatile(
      //"prfm   pldl1strm, [%0]     \n"
      "0: \n"
      ...
      "sri    v1.4s, v2.4s, #2    \n"
      "sri    v0.4s, v1.4s, #4    \n"
      // v0 contains 8 signbits in each part
      "ld1    {v1.4s, v2.4s, v3.4s, v4.4s}, [%0], #64    \n"
      "sri    v1.4s, v3.4s, #1    \n"
      "sri    v2.4s, v4.4s, #1    \n"
      "sri    v1.4s, v2.4s, #2    \n"
...
// BitFieldInsert. From the manual at
