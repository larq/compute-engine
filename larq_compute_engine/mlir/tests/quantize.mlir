...
^bb0(%arg0: tensor<1x224x224x3x!quant.uniform<u8:f32, 7.812500e-03:128>>, %arg1: tensor<32x3x3x1xi32>):
  ...
  %2 = "tfl.dequantize"(%arg0) : (tensor<1x224x224x3x!quant.uniform<u8:f32, 7.812500e-03:128>>) -> tensor<1x224x224x3xf32>
  %3 = "tf.LceBconv2d"(%2, %arg1, %cst0, %cst1) {activation = "NONE", data_format = "NHWC", dilations = [1, 2, 3, 1], filter_format = "OHWI_PACKED", pad_values = 0 : i32, padding = "SAME", strides = [1, 4, 5, 1]} : (tensor<1x224x224x3xf32>, tensor<32x3x3x1xi32>, tensor<32xf32>, tensor<32xf32>) -> tensor<1x112x112x32xf32>
