...
^bb0(%arg0: tensor<1x224x224x3x!quant.uniform<u8:f32, 7.812500e-03:128>>, %arg1: tensor<32x3x3x1xi32>):
  ...
  %2 = "tfl.dequantize"(%arg0) : (tensor<1x224x224x3x!quant.uniform<u8:f32, 7.812500e-03:128>>) -> tensor<1x224x224x3xf32>
  %3 = "tf.LceBconv2d"(%2, %arg1, %cst0, %cst1) {activation = "NONE", channels_in = 3 : i32, data_format = "NHWC", dilations = [1, 2, 3, 1], filter_format = "OHWI_PACKED", pad_values = 0 : i32, padding = "SAME", strides = [1, 4, 5, 1]} : (tensor<1x224x224x3xf32>, tensor<32x3x3x1xi32>, tensor<32xf32>, tensor<32xf32>) -> tensor<1x112x112x32xf32>
  %4 = "tfl.quantize"(%3) {qtype = tensor<1x112x112x32x!quant.uniform<u8:f32, 0.023528476789885875>>} : (tensor<1x112x112x32xf32>) -> tensor<1x112x112x32x!quant.uniform<u8:f32, 0.023528476789885875>>
  return %4 : tensor<1x112x112x32x!quant.uniform<u8:f32, 0.023528476789885875>>

// CHECK: %[[cst0:.*]] = constant dense<-1.23697901> : tensor<32xf32>
// CHECK: %[[cst1:.*]] = constant dense<1.10976315> : tensor<32xf32>
// CHECK: %[[conv:.*]] = "tf.LceBconv2d"(%arg0, %arg1, %[[cst0]], %[[cst1]])
// CHECK: return %[[conv]] : tensor<1x112x112x32x!quant.uniform<u8:f32, 0.023528476789885875>>
}
