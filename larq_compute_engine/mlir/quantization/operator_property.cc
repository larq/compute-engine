...
//
// This file is forked from tensorflow/lite/tools/optimize/operator_property.cc
// The only modifications are the if-block at the top of GetOperatorProperty
// And the flexbuffer.h include
//
...
OperatorProperty GetOperatorProperty(const ModelT* model, int subgraph_index,
  ...
  if (op_code == BuiltinOperator_CUSTOM) {
    OperatorT* op =
        model->subgraphs.at(subgraph_index)->operators[op_index].get();
    auto custom_name = model->operator_codes[op->opcode_index]->custom_code;
    if (custom_name == "LceBconv2d") {
      // Check for zero-padding, in which case int8 is not supported
      const uint8_t* buffer_t = op->custom_options.data();
      const size_t length = op->custom_options.size();
      const flexbuffers::Map& m =
          flexbuffers::GetRoot(buffer_t, length).AsMap();
      if ((m["padding"].ToString() == "SAME" ||
           m["padding"].ToString() == "same") &&
          (m["pad_values"].IsNull() || m["pad_values"].AsInt64() != 1)) {
        property.quantizable = false;
        return property;
      }

      // Note: the same node will be requested many times, in different passes
      // of the quantizer. In some passes, it might be that an input is already
      // int8 quantized while an output is not, and so on.
      // Even when an input is already int8, we should still return that it is
      // quantizable.

      auto input_type =
          model->subgraphs.at(subgraph_index)->tensors[op->inputs[0]]->type;
      auto output_type =
          model->subgraphs.at(subgraph_index)->tensors[op->outputs[0]]->type;

      if (input_type == TensorType_INT32 && output_type == TensorType_INT32) {
        // Pure riptide style, nothing to quantize.
        property.quantizable = false;
        return property;
      }

      // No zero-padding, so we support int8
      property.quantizable = true;

      // Note: never quantize the filter and post_xxx!
      if (input_type != TensorType_INT32) {
        property.inputs = {{0, {}}};
      }
      if (output_type != TensorType_INT32) {
        property.outputs = {{0, {}}};
      }
